            ...
            {homeData.appointment 
                ? <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <p className="font-medium text-lg leading-tight">{homeData.appointment.monitor.user.name}</p>
                        <p className="opacity-70 text-sm mt-0.5">
                            {activeProfile == "aluno" 
                                ? homeData.appointment.monitor.department ?? "Undefined"
                                : "aluno"
                            }
                        </p>
                        
                        <p className="font-mono text-sm my-3.5 opacity-90">
                            {homeData.appointment.date.toLocaleDateString()}
                        </p>

                        <p className="text-sm opacity-80 line-clamp-1">{homeData.appointment.topic}</p> 
                    </div>
                </div>
